...
p.addParameter('pixel_type', 'gridded');%'gridded');
...
if ~onCluster
    for a=1:numel(FILES)
        ...
        if datenum(date_string,'yyyymmdd') >= datenum(start_date) && datenum(date_string,'yyyymmdd') <= datenum(end_date)
            ...
            for iparam = 1:length(ErrorData)
                if strcmpi(pixel_type,'native')
                    Data_to_save = ErrorData(iparam).Delta;
                else
                    Data_to_save = ErrorData(iparam).DeltaGrid;
                end
                ...
                if length(ErrorData) == 1
                    savename = savename;
                elseif length(ErrorData) == 2
                    % add number suffix
                    [~, name, ext] = fileparts(savename);
                    savename = strcat(name, '_', num2str(iparam), ext);
                else
                    fprintf('Error: wrong array size of %s\n', fullfile(mat_file_dir,FILES(a).name));
                end
    ...
    parfor a=1:numel(FILES)
        ...
        if datenum(date_string,'yyyymmdd') >= datenum(start_date) && datenum(date_string,'yyyymmdd') <= datenum(end_date)
            ...
            if strcmpi(pixel_type,'native')
                Data_to_save = D.ErrorData.Delta;
                [sz1, sz2]   = size(Data_to_save);
            else
                Data_to_save = D.ErrorData.DeltaGrid;
                [sz1, sz2]   = size(Data_to_save);
            end 
            ...
            elseif sz1 == 1 && sz2 == 2
                savename_1 = strcat(name,'_1',ext);
                savename_2 = strcat(name,'_2',ext);
                if strcmpi(output_type,'hdf')
                    make_hdf_file(Data_to_save(1), vars, attr, full_save_dir, savename_1, pixel_type, overwrite, git_heads, DEBUG_LEVEL);
                    make_hdf_file(Data_to_save(2), vars, attr, full_save_dir, savename_2, pixel_type, overwrite, git_heads, DEBUG_LEVEL);
                elseif strcmpi(output_type,'txt')
                    make_txt_file(Data_to_save(1), vars, attr, date_string, full_save_dir, savename_1, overwrite, DEBUG_LEVEL);
                    make_txt_file(Data_to_save(2), vars, attr, date_string, full_save_dir, savename_2, overwrite, DEBUG_LEVEL);
                end
            else
                disp(sz1);
                disp(sz2);
...
if reprocessed
    attr =  BEHR_publishing_attribute_subtable('pub-insitu', 'struct');
    savename = strrep(savename, 'BEHR','BEHR-InSitu');
else
    attr = BEHR_publishing_attribute_subtable('pub', 'struct');
end
...
gridded_vars = BEHR_publishing_gridded_fields_sub.all_gridded_vars;
...
for d=1:numel(Data_in)
    ...
    %    disp(Data_in(d).(swath_attr_fields{a}));
